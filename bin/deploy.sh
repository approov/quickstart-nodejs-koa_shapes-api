...
Setup_Configuration() {
  ...
  local tag=$(echo ${BUILD_RELEASE_FROM} | sed -r "s|/|-|")

  DOCKER_IMAGE="${APP_VENDOR}/${DOMAIN}:${tag}_${DATETIME}"
}
...
Deploy_Release() {
  ...
  # No matter which env we deploy too, the docker image has a timestamp as the
  # tag for each release, therefore we always need to update the .env.${RELASE_TAG}.deploy
  # file.
